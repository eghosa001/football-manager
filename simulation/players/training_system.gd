...
func run_week(world: Dictionary, club_id: String, seed: int) -> Dictionary:
	...
	for player in world.get("players", []):
		...
		var development_score := potential_gap * 0.02 * age_factor * (0.6 + coaching / 125.0) * (0.7 + facilities / 170.0) * (0.7 + professionalism / 170.0) * intensity * development_multiplier
		...
		if work_days > 0 and SeededRngClass.unit_for(seed,key) < injury_risk*float(work_days)/5.0:
			...
			injuries += 1
	_apply_tactical_familiarity(club,mix,intensity)
	return {"players_improved":improved,"individual_focus_gains":individual_focus_gains,"learned_traits":learned_traits,"fatigue_added":fatigue_added,"training_injuries":injuries,"coaching_quality":coaching,"session_mix":mix.duplicate(true),"tactical_familiarity":float(club.get("tactic",{}).get("familiarity",0.0))}
...
func _session_mix(sessions: Array) -> Dictionary:
	...
	for raw in sessions:
		var session := String(LEGACY_ALIASES.get(String(raw),String(raw)))
		if counts.has(session):
			counts[session] = int(counts[session])+1
...
func _apply_session_attribute_bias(player: Dictionary, mix: Dictionary, gain: int) -> void:
	...
	var targets: Array = []
	if int(counts.attacking)>0:
		targets.append_array(["finishing","off_the_ball","composure","dribbling"])
	if int(counts.defending)>0:
		targets.append_array(["tackling","marking","positioning","concentration"])
	if int(counts.possession)>0:
		targets.append_array(["passing","first_touch","vision","decisions"])
	if int(counts.technical)>0:
		targets.append_array(["technique","passing","dribbling","first_touch"])
	if int(counts.physical)>0:
		targets.append_array(["pace","acceleration","stamina","strength","agility"])
	if int(counts.set_piece)>0:
		targets.append_array(["set_pieces","crossing","penalties"])
	for name in targets:
		if attrs.has(String(name)):
			attrs[String(name)] = clampi(int(attrs[String(name)])+1,1,100)
...
func _apply_tactical_familiarity(club: Dictionary, mix: Dictionary, intensity: float) -> void:
	if not club.has("tactic"):
		return
	var counts: Dictionary = mix.counts
	var tactical_sessions := int(counts.tactical)+int(counts.match_preparation)+int(counts.possession)+int(counts.set_piece)
	if tactical_sessions <= 0:
		return
	club.tactic.familiarity = clampf(float(club.tactic.get("familiarity",50.0))+float(tactical_sessions)*0.7*intensity,0.0,100.0)
