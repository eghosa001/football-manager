...
func create(world: Dictionary, club: Dictionary, seed: int, unique_id: String, index: int = 0, age_override: int = -1) -> Dictionary:
	...
	var key := _stable_key(unique_id)
	var age := age_override if age_override >= 0 else 15 + int(SeededRngClass.value_for(seed, key + 8) % 3)
	...
	var used := _used_names(world)
	var identity: Dictionary = realism.generated_name(data, nationality, seed, key + 6 + index * 17, used, unique_id)
	var ca_pa := _ability_profile(world, club, nationality, seed, key)
	var ca := int(ca_pa.ca)
	var potential := int(ca_pa.pa)
	var height := _height_for_position(position, seed, key + 7)
	var player := {
		"id":unique_id,
		"club_id":String(club.get("id", "")),
		"country_id":nationality,
		"nationality_id":nationality,
		"second_nationality_id":second_nationality,
		...
		"current_ability":ca,
		"potential":potential,
		"development_ceiling":potential,
		"base_potential":potential,
		"development_trajectory":_trajectory(seed, key + 15),
		"fitness":100,
		"morale":70,
		"retired":false,
		"injured_days":0,
		...
		"career_seasons":0,
		"preferred_foot":FEET[int(SeededRngClass.value_for(seed,key+9)%FEET.size())],
		"weak_foot":30 + int(SeededRngClass.value_for(seed,key+19)%56),
		"height_cm":height,
		"weight_kg":_weight(height,seed,key+10),
		"body_type":BODY_TYPES[int(SeededRngClass.value_for(seed,key+11)%BODY_TYPES.size())],
		"growth_stage":"adolescent" if age <= 16 else "late_adolescent",
		"physical_maturity":clampf(0.66 + float(age - 15) * 0.08 + float(SeededRngClass.value_for(seed,key+20)%9)/100.0,0.62,0.92),
		"homegrown":nationality == country_id,
		"squad_status":"academy",
		"newgen":true,
		"fictional_identity":true,
	}
	player["hidden_attributes"] = _hidden_attributes(seed, key, ca, potential)
	player["personality"] = {
		"professionalism":int(player.hidden_attributes.professionalism),
		"ambition":int(player.hidden_attributes.ambition),
		"loyalty":int(player.hidden_attributes.loyalty),
	}
	player["attributes"] = _attributes(position, ca, age, float(player.physical_maturity), seed, key)
...
func mature_physical(player: Dictionary, seed: int) -> void:
	if not bool(player.get("newgen", false)):
		return
	var age := int(player.get("age", 18))
	var maturity := float(player.get("physical_maturity", 0.8))
	if maturity >= 1.0 or age >= 22:
		player["physical_maturity"] = 1.0
		player["growth_stage"] = "mature"
		return
	var key := _stable_key(String(player.get("id", ""))) + age * 313
	var increment := 0.05 + float(SeededRngClass.value_for(seed,key)%5)/100.0
	var next := minf(1.0, maturity + increment)
	var attrs: Dictionary = player.get("attributes", {})
	var delta := next - maturity
	...
	player["physical_maturity"] = next
	player["growth_stage"] = "mature" if next >= 0.98 else "developing"
...
func _ability_profile(world: Dictionary, club: Dictionary, country_id: String, seed: int, key: int) -> Dictionary:
	...
	var recruitment := int(club.get("youth_recruitment", club.get("academy", {}).get("recruitment", 50)))
	var coaching := int(club.get("academy", {}).get("coaching", 50))
	var rep := int(club.get("reputation", 50))
	var talent := _country_talent(world, country_id)
	var environment := academy * 0.24 + recruitment * 0.28 + coaching * 0.16 + rep * 0.12 + talent * 100.0 * 0.20
	var roll := float(SeededRngClass.value_for(seed,key+21)%10000)/10000.0
	var tier_bonus := 0
	if roll > 0.998: tier_bonus = 24
	elif roll > 0.985: tier_bonus = 14
	elif roll > 0.93: tier_bonus = 7
	var ca := clampi(int(environment * 0.58) + int(SeededRngClass.value_for(seed,key)%23)-11 + tier_bonus/3, 18, 68)
	var gap := 10 + int(SeededRngClass.value_for(seed,key+1)%30) + int(environment/11.0) + tier_bonus
	var pa := clampi(ca + gap, ca, 99)
	return {"ca":ca,"pa":pa}

func _attributes(position: String, ca: int, age: int, maturity: float, seed: int, key: int) -> Dictionary:
	var profile = PlayerProfileClass.new()
	var attrs := {}
	var names: Array = profile.TECHNICAL + profile.MENTAL + profile.PHYSICAL + profile.GOALKEEPING
	var biases: Dictionary = POSITION_BIASES.get(position, {})
	for i in range(names.size()):
		var name := String(names[i])
		var base := ca + int(biases.get(name, 0)) + int(SeededRngClass.value_for(seed,key+200+i*13)%15)-7
		if name in profile.PHYSICAL:
			base = int(round(float(base) * lerpf(0.78,1.0,maturity)))
		if position != "GK" and name in profile.GOALKEEPING:
			base = min(base, 28 + int(SeededRngClass.value_for(seed,key+900+i)%13))
		if position == "GK" and name in profile.TECHNICAL and name not in ["passing","first_touch","technique"]:
			base -= 8
...
func _hidden_attributes(seed: int, key: int, ca: int, pa: int) -> Dictionary:
	...
	for i in range(PlayerProfileClass.HIDDEN.size()):
		var name := String(PlayerProfileClass.HIDDEN[i])
		result[name] = 25 + int(SeededRngClass.value_for(seed,key+1200+i*29)%66)
	# Elite prospects are not automatically perfect personalities, but exceptional
	# potential slightly raises determination-adjacent developmental qualities.
	if pa - ca >= 30:
...
func _nationality(world: Dictionary, home_country: String, seed: int, key: int) -> String:
	var recruitment_reach := 0.84
	if SeededRngClass.unit_for(seed,key) < recruitment_reach and home_country != "": return home_country
...
func _country_talent(world: Dictionary, country_id: String) -> float:
	for country in world.get("countries",[]):
		...
		var national_success := float(country.get("national_success",50))
		var league_rep := float(country.get("league_reputation",50))
...
func _trajectory(seed: int, key: int) -> String:
	var roll := int(SeededRngClass.value_for(seed,key)%100)
...
func _used_names(world: Dictionary) -> Dictionary:
	...
	for player in world.get("players",[]):
		var full := (String(player.get("first_name",""))+" "+String(player.get("last_name",""))).strip_edges()
		if full != "": used[full] = true
	for member in world.get("staff",[]):
		var full := String(member.get("name","")).strip_edges()
		if full != "": used[full] = true
	return used
