...
# Injuries are generated by the authoritative medical/training/match systems.
# Annual lifecycle rollover no longer performs an unrelated second injury lottery.
...
func train_player(player: Dictionary, focus: String, intensity: float, seed: int) -> int:
	...
	var gained := super.train_player(player, focus, intensity, seed)
	var abilities = SpecialAbilityServiceClass.new()
	var multiplier := abilities.development_multiplier(player)
	if multiplier <= 1.0:
		return gained
	var potential := int(player.get("potential", before))
	var room := maxi(0, potential - int(player.get("current_ability", before)))
	if room <= 0:
		return gained
	# Prodigy improves the rate, not the ceiling. The extra training point is
	# deterministic and still constrained by potential.
	var extra_chance := clampf((multiplier - 1.0) * clampf(intensity, 0.0, 1.0), 0.0, 0.45)
	var key := _stable_key(String(player.get("id", ""))) + int(player.get("career_seasons", 0)) * 401 + 17777
...
func _apply_development(player: Dictionary, world: Dictionary, seed: int) -> Dictionary:
	...
	if multiplier <= 1.0:
		return record
	var after := int(player.get("current_ability", before))
	var natural_gain := maxi(0, after - before)
	var potential := int(player.get("potential", after))
	var room := maxi(0, potential - after)
	if natural_gain <= 0 or room <= 0:
		return record
	var extra := mini(room, int(round(float(natural_gain) * (multiplier - 1.0))))
	if extra <= 0 and multiplier >= 1.30:
		extra = 1
	...
	if extra > 0:
		...
		if not history.is_empty():
			history[history.size() - 1] = record.duplicate(true)
	return record
