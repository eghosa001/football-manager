...
func candidates(world: Dictionary, hiring_club_id: String, role: String, limit: int = 20) -> Array:
	var rows: Array = []
	var contracts_by_staff: Dictionary = {}
	for contract in world.get("staff_contracts", []):
		var staff_id := String(contract.get("staff_id", ""))
		if staff_id != "": contracts_by_staff[staff_id] = contract
	var contract_service = StaffContractsClass.new()
	...
	for member in world.get("staff", []):
		...
		var contract: Dictionary = contracts_by_staff.get(String(member.get("id", "")), {})
		var compensation := _compensation_from_contract(contract, season_year) if current_club != "" else 0
		rows.append({"staff_id":String(member.id),"name":String(member.get("name","")),"ability":ability,"current_club_id":current_club,"weekly_wage":contract_service.recommended_wage(member),"compensation":compensation,"score":ability*1.0-(compensation/100000.0)})
...
func _remaining_compensation(world: Dictionary, staff_id: String) -> int:
	return _compensation_from_contract(_contract(world, staff_id), int(world.get("season_year", 2026)))

func _compensation_from_contract(contract: Dictionary, season_year: int) -> int:
	if contract.is_empty(): return 0
	var years := maxi(0,int(contract.get("end_year",0))-season_year)
