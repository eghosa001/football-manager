...
func create_bracket(club_ids: Array, seed: int) -> Dictionary:
	...
	var size := 1
	while size < entrants.size(): size *= 2
	while entrants.size() < size: entrants.append("")
...
func advance_round(bracket: Dictionary, results: Array) -> Dictionary:
	...
	for i in range(bracket.matches.size()):
		...
		var home := String(match.home); var away := String(match.away)
	...
	if winners.size() == 1:
		...
		return bracket
	bracket.round = int(bracket.round) + 1; bracket.entrants = winners; bracket.matches = _pair(winners)
	return bracket

func _pair(entrants: Array) -> Array:
	var matches: Array = []
	for i in range(0, entrants.size(), 2): matches.append({"home":String(entrants[i]),"away":String(entrants[i+1])})
	return matches
