...
func register_callups(world: Dictionary, country_id: String, competition_id: String, size: int = 23) -> Dictionary:
	ensure_world(world)
	var squad := select_squad(world, country_id, size)
	var record := {"country_id":country_id,"competition_id":competition_id,"player_ids":squad,"season_year":int(world.get("season_year",2026))}
	world.international_callups.append(record)
	return record

func qualifying_groups(country_ids: Array, group_count: int, competition_id: String) -> Dictionary:
	var groups := GroupStageClass.new().seed_groups(country_ids, group_count)
	return {"competition_id":competition_id,"groups":groups,"fixtures":GroupStageClass.new().fixtures_for_groups(groups, competition_id)}
