...
func seed_groups(club_ids: Array, group_count: int = 8) -> Array:
	var count := maxi(1, group_count)
	var groups: Array = []
	for i in range(count):
		groups.append([])
	var ordered := club_ids.duplicate()
	ordered.sort()
	for i in range(ordered.size()):
		groups[i % count].append(ordered[i])
	return groups
	...
	for group_index in range(groups.size()):
		...
		for i in range(clubs.size()):
			for j in range(i + 1, clubs.size()):
				for leg in range(2):
					var home := clubs[i] if leg == 0 else clubs[j]
					var away := clubs[j] if leg == 0 else clubs[i]
...
func qualifiers(groups: Array, fixtures: Array, per_group: int = 2) -> Array:
	...
	for group_index in range(groups.size()):
		...
		for fixture in fixtures:
			if int(fixture.get("group", 0)) == group_index + 1:
				group_fixtures.append(fixture)
		var table: Array = LeagueTableClass.build(groups[group_index], group_fixtures, 3, 1)
		for i in range(mini(per_group, table.size())):
			result.append(String(table[i].club_id))
	return result
