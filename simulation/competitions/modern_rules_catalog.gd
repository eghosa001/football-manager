...
const UNIVERSAL_MATCH_RULES := {"substitutes_allowed":5,"substitution_windows":3,"extra_time_substitute":1,"extra_time_minutes":30,"away_goals":false,"penalties":true,"replays":false,"points_win":3,"points_draw":1}
...
const EUROPEAN_LEAGUE_PHASES := {1:{"teams":36,"matches_per_club":8,"home_matches":4,"away_matches":4,"direct_round_of_16":[1,8],"knockout_playoff":[9,24],"eliminated":[25,36]},2:{"teams":36,"matches_per_club":8,"home_matches":4,"away_matches":4,"direct_round_of_16":[1,8],"knockout_playoff":[9,24],"eliminated":[25,36]},3:{"teams":36,"matches_per_club":6,"home_matches":3,"away_matches":3,"direct_round_of_16":[1,8],"knockout_playoff":[9,24],"eliminated":[25,36]}}
	...
	for competition in world.get("competitions",[]):
		...
		if bool(competition.get("continental",false)) and String(competition.get("continental_region","")) == "europe":
			...
			if EUROPEAN_LEAGUE_PHASES.has(continental_tier):
				competition["modern_league_phase"] = EUROPEAN_LEAGUE_PHASES[continental_tier].duplicate(true); competition["away_goals"] = false; competition["rules"] = _merge_rules(competition.get("rules",{}),UNIVERSAL_MATCH_RULES)
...
func _apply_universal_rules(competition: Dictionary) -> void:
	...
	competition["rules"] = _merge_rules(competition.get("rules",{}),UNIVERSAL_MATCH_RULES); competition.rules["away_goals"] = false; competition["rules_standard_version"] = STANDARD_VERSION
...
func _apply_domestic_structure(competition: Dictionary, expected: Dictionary) -> int:
	...
	for source_key in mapping.keys():
		...
		if competition.get(target_key) != value:
			competition[target_key] = value.duplicate(true) if value is Array or value is Dictionary else value; changes += 1
...
func _merge_rules(existing: Dictionary, defaults: Dictionary) -> Dictionary:
	...
	merged["away_goals"] = false; merged["substitutes_allowed"] = 5; merged["substitution_windows"] = 3; merged["extra_time_substitute"] = 1; merged["extra_time_minutes"] = 30
	return merged
