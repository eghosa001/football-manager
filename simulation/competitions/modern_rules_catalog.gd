...
# Domestic movement structures represented by the shipped database. These are
# intentionally stored by country/tier so old saves can be migrated safely.
const DOMESTIC_STRUCTURE := {
	"eng": {
		1:{"teams":20,"promotion":0,"automatic_promotion":0,"playoff_promotion":0,"relegation":3,"relegation_playoff":0},
		2:{"teams":24,"promotion":3,"automatic_promotion":2,"playoff_promotion":1,"playoff_places":[3,6],"relegation":3},
		3:{"teams":24,"promotion":3,"automatic_promotion":2,"playoff_promotion":1,"playoff_places":[3,6],"relegation":4},
	},
	"esp": {
		1:{"teams":20,"promotion":0,"relegation":3},
		2:{"teams":22,"promotion":3,"automatic_promotion":2,"playoff_promotion":1,"playoff_places":[3,6],"relegation":4},
	},
	"deu": {
		1:{"teams":18,"promotion":0,"relegation":2,"relegation_playoff":1},
		2:{"teams":18,"promotion":2,"automatic_promotion":2,"promotion_playoff_vs_upper":1,"relegation":2,"relegation_playoff":1},
	},
	"fra": {
		1:{"teams":18,"promotion":0,"relegation":2,"relegation_playoff":1},
		2:{"teams":18,"promotion":2,"automatic_promotion":2,"promotion_playoff_vs_upper":1,"relegation":3},
	},
	"ita": {
		1:{"teams":20,"promotion":0,"relegation":3},
		2:{"teams":20,"promotion":3,"automatic_promotion":2,"playoff_promotion":1,"playoff_places":[3,8],"relegation":3},
	},
	"nld": {
		1:{"teams":18,"promotion":0,"relegation":2,"relegation_playoff":1},
		2:{"teams":20,"promotion":2,"automatic_promotion":2,"promotion_playoff_vs_upper":1,"relegation":0},
	},
	"bra": {
		1:{"teams":20,"promotion":0,"relegation":4},
		# 2026 Serie B: top two automatic, positions 3-6 contest two additional places.
		2:{"teams":20,"promotion":4,"automatic_promotion":2,"playoff_promotion":2,"playoff_places":[3,6],"relegation":4},
	},
	"arg": {
		# The Argentine pyramid changes format frequently; retain the launch-sized
		# top division but explicitly mark it as a season-profiled competition so
		# rules can be migrated without corrupting existing careers.
		1:{"teams":30,"promotion":0,"relegation":2,"season_profiled":true},
	},
}
...
func apply_to_world(world: Dictionary) -> Dictionary:
	...
	for competition in world.get("competitions", []):
		...
		if String(competition.get("competition_type", "league")) == "league":
			...
			var tier := int(competition.get("tier", 1))
			...
			if not expected.is_empty():
				...
				if expected.has("teams") and competition.get("club_ids", []).size() != int(expected.teams):
					warnings.append("%s has %d clubs; modern profile expects %d" % [String(competition.get("id","")), competition.get("club_ids", []).size(), int(expected.teams)])
		if bool(competition.get("continental", false)) and String(competition.get("continental_region", "")) == "europe":
			var continental_tier := int(competition.get("continental_tier", 1))
...
func modern_rules_for(competition: Dictionary) -> Dictionary:
	...
	for key in ["substitutes_allowed","substitution_windows","extra_time_substitute","extra_time_minutes","away_goals","penalties","replays","points_win","points_draw","tie_breakers"]:
		if competition.has(key): rules[key] = competition[key]
	# Shipped modern competitions must never silently reactivate away goals.
...
func validate_world(world: Dictionary) -> Array:
	var errors: Array = []
	for competition in world.get("competitions", []):
		var rules := modern_rules_for(competition)
		if int(rules.get("substitutes_allowed", 0)) != 5:
			errors.append("%s must allow five substitutes" % String(competition.get("id","")))
		if int(rules.get("substitution_windows", 0)) != 3:
			errors.append("%s must use three regulation-time substitution windows" % String(competition.get("id","")))
		if bool(rules.get("away_goals", false)):
			errors.append("%s uses obsolete away-goals rule" % String(competition.get("id","")))
		...
		if String(competition.get("competition_type", "league")) == "league" and not expected.is_empty():
			...
			for field in ["promotion","automatic_promotion","playoff_promotion","relegation","relegation_playoff","promotion_playoff_vs_upper"]:
				if not expected.has(field): continue
				var actual_key := {"promotion":"promotion_places","automatic_promotion":"automatic_promotion_places","playoff_promotion":"playoff_promotion_places","relegation":"relegation_places","relegation_playoff":"relegation_playoff_places","promotion_playoff_vs_upper":"promotion_playoff_vs_upper"}[field]
...
func _apply_universal_rules(competition: Dictionary) -> void:
	for key in UNIVERSAL_MATCH_RULES.keys():
		if key == "away_goals":
			competition[key] = false
		elif not competition.has(key):
			competition[key] = UNIVERSAL_MATCH_RULES[key]
	competition["rules"] = _merge_rules(competition.get("rules", {}), UNIVERSAL_MATCH_RULES)
...
func _apply_domestic_structure(competition: Dictionary, expected: Dictionary) -> int:
	var changes := 0
	var mapping := {
		"promotion":"promotion_places",
		"automatic_promotion":"automatic_promotion_places",
		"playoff_promotion":"playoff_promotion_places",
		"playoff_places":"playoff_places",
		"promotion_playoff_vs_upper":"promotion_playoff_vs_upper",
		"relegation":"relegation_places",
		"relegation_playoff":"relegation_playoff_places",
	}
	for source_key in mapping.keys():
		if not expected.has(source_key): continue
		var target_key := String(mapping[source_key])
		var value = expected[source_key]
