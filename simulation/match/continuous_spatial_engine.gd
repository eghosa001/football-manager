...
# Legacy dense interpolation path (kept for compatibility + viewed-match smoothing).
...
# True continuous simulation: every player moves every tick.
# Tactics change movement first; goals/shots emerge from positions, not multipliers.
func simulate_continuous(home_lineup: Array, away_lineup: Array, seed: int, home_tactic: Dictionary = {}, away_tactic: Dictionary = {}, ticks: int = 540, previous_state: Dictionary = {}) -> Dictionary:
	...
	if previous_state.has("home_positions"):
		for id in home_pos.keys():
			if previous_state["home_positions"].has(id):
				home_pos[id] = (previous_state["home_positions"][id] as Dictionary).duplicate(true)
	if previous_state.has("away_positions"):
		for id in away_pos.keys():
			if previous_state["away_positions"].has(id):
				away_pos[id] = (previous_state["away_positions"][id] as Dictionary).duplicate(true)
	var loads := {"home": {}, "away": {}}
	for player in home_lineup:
		loads["home"][String(player.id)] = PhysicalModelClass.initial_load() if not _has_load(previous_state, "home", String(player.id)) else (previous_state["loads"]["home"][String(player.id)] as Dictionary).duplicate(true)
	for player in away_lineup:
		loads["away"][String(player.id)] = PhysicalModelClass.initial_load() if not _has_load(previous_state, "away", String(player.id)) else (previous_state["loads"]["away"][String(player.id)] as Dictionary).duplicate(true)
	...
	for tick in range(ticks):
		...
		# Ball travel toward target; interception only during true travel
		# (not at the owner's feet) and only by opponents.
		var before := ball.duplicate(true)
		...
		if ball_travelling and SpatialStateClass.distance(ball, owner_pos) > 2.0:
	...
	# Deterministic summary for distribution tests.
...
func _anchors(lineup: Array, home: bool, profile: Dictionary) -> Dictionary:
	...
	var line: float = profile.line_height
	for i in range(lineup.size()):
		var id := String(lineup[i].id)
		if i == 0:
			var gx := 3.0 if home else PITCH_LENGTH - 3.0
			positions[id] = {"x": gx, "y": PITCH_WIDTH * 0.5}
			continue
		var row := float((i - 1) % 10) / 9.0
		var col := float((i * 37) % 11) / 10.0
...
func _move_side(lineup: Array, own: Dictionary, opp: Dictionary, ball: Dictionary, profile: Dictionary, loads: Dictionary, marking: Dictionary, in_possession: bool, dt: float, tick: int) -> void:
	...
	for i in range(lineup.size()):
		var id := String(lineup[i].id)
		if i == 0:
			continue # keeper handled separately
		var pos: Dictionary = own[id]
		var anchor: Dictionary = _role_anchor(lineup[i], i, profile)
		var target := anchor.duplicate(true)
		# Off-ball runs / defensive tracking toward ball or mark.
		if in_possession:
			...
			if ball_d < float(profile.press_trigger):
				target = ball.duplicate(true) # pressing trigger: close at closing_speed
			elif marking.has(id):
				var aid: String = marking[id]
				var mark_pos: Dictionary = _opp_position(opp, aid, pos)
				target = _blend(anchor, mark_pos, 0.55)
			else:
				target = _blend(anchor, ball, 0.25) # compactness/recovery run
...
func _check_interception(ball: Dictionary, home_pos: Dictionary, away_pos: Dictionary, owner: Dictionary) -> Dictionary:
	for side in ["home", "away"]:
		if side == String(owner.get("side", "")):
			continue
		var positions: Dictionary = home_pos if side == "home" else away_pos
		for id in positions.keys():
			if SpatialStateClass.distance(ball, positions[id]) < INTERCEPT_RADIUS:
				return {"side": side, "id": id}
	return {"side": "", "id": ""}
...
func _decide_action(ball: Dictionary, owner: Dictionary, possession: String, home_lineup: Array, away_lineup: Array, home_pos: Dictionary, away_pos: Dictionary, home_profile: Dictionary, away_profile: Dictionary, loads: Dictionary, seed: int, tick: int) -> Dictionary:
	...
	var positions: Dictionary = home_pos if possession == "home" else away_pos
	var profile: Dictionary = home_profile if possession == "home" else away_profile
	var goal_x := PITCH_LENGTH if possession == "home" else 0.0
	var dist_goal := absf(goal_x - float(ball.x))
	var direct: float = profile.directness
	var r := SeededRngClass.unit_for(seed, 20000 + tick)
	# Tempo already gates decision cadence; directness picks target depth.
	if dist_goal < 28.0 and r < 0.45:
		var xg := clampf(0.45 - dist_goal / 90.0, 0.03, 0.42) * float(profile.execution)
		var scored := SeededRngClass.unit_for(seed, 21000 + tick) < xg
		var target := {"x": goal_x, "y": PITCH_WIDTH * 0.5}
		return {"possession": possession, "owner_id": owner.id, "target": target, "event": "goal" if scored else "shot", "xg": xg}
	# Pass: forward-biased by directness, never instant teleport (ball travel handles it).
	var mates: Array = []
	for player in team:
		if String(player.id) != owner.id:
			mates.append(player)
	if mates.is_empty():
		return {"possession": possession, "owner_id": owner.id, "target": ball, "event": ""}
	var direction := 1.0 if possession == "home" else -1.0
	# Dribble progression: carrier advances toward goal, creating shooting range.
	# Keeps ball travel continuous rather than pass-only oscillation.
	if r > 0.62:
		var advance := 5.0 + direct * 6.0
		var dribble_target := SpatialStateClass.clamp_position({"x": float(ball.x) + direction * advance, "y": clampf(float(ball.y) + (SeededRngClass.unit_for(seed, 23000 + tick) - 0.5) * 8.0, 2.0, PITCH_WIDTH - 2.0)})
		return {"possession": possession, "owner_id": owner.id, "target": dribble_target, "event": ""}
	var best: Dictionary = mates[0]
	var best_score := -INF
	for mate in mates:
		var pos: Dictionary = positions.get(String(mate.id), ball)
		var forward := (float(pos.x) - float(ball.x)) * direction
		var score := forward * (0.4 + direct) - SpatialStateClass.distance(ball, pos) * 0.12
...
func _role_anchor(player: Dictionary, index: int, profile: Dictionary) -> Dictionary:
	# Width + line already encode instructions; overlap adds wide full-back push.
	var base := {"x": PITCH_LENGTH * 0.5, "y": PITCH_WIDTH * 0.5}
	var pos_name := String(player.get("position", "MC"))
	var wide := float(profile.width)
	var line := float(profile.line_height)
	var x := lerpf(18.0, PITCH_LENGTH - 18.0, line)
	if pos_name in ["ST", "AMR", "AML", "AMC"]:
		x += 14.0
	elif pos_name in ["GK"]:
		x = 4.0
	elif pos_name in ["DC", "DR", "DL"]:
		x -= 16.0
	var y := PITCH_WIDTH * 0.5
	if pos_name in ["DR", "MR", "AMR", "WBR"]:
		y = lerpf(PITCH_WIDTH * 0.5, PITCH_WIDTH * (0.5 + wide * 0.6), 0.8)
	elif pos_name in ["DL", "ML", "AML", "WBL"]:
		y = lerpf(PITCH_WIDTH * 0.5, PITCH_WIDTH * (0.5 - wide * 0.6), 0.8)
...
func _is_home_side(own: Dictionary, pos: Dictionary) -> bool:
	return float(pos.get("x", 52.5)) < PITCH_LENGTH * 0.5 or own.size() > 0
...
func _copy_positions(positions: Dictionary) -> Dictionary:
	var result := {}
	for id in positions.keys():
		result[id] = (positions[id] as Dictionary).duplicate(true)
	return result
...
func _copy_loads(loads: Dictionary) -> Dictionary:
	...
	for side in loads.keys():
		result[side] = {}
		for id in (loads[side] as Dictionary).keys():
			result[side][id] = ((loads[side] as Dictionary)[id] as Dictionary).duplicate(true)
	return result
...
func _total_distance(side_loads: Dictionary) -> float:
	...
	for id in side_loads.keys():
		total += float((side_loads[id] as Dictionary).get("distance", 0.0))
	return total
