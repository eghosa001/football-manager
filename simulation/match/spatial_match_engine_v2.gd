...
func _choose_action(actor: Dictionary, state: Dictionary, seed: int, index: int) -> String:
	...
	var distance_to_goal := absf(goal_x - float(state.ball.x)); var pressure := _pressure(state, String(state.possession_side), state.ball); var attrs: Dictionary = actor.get("attributes", {})
	var finishing := float(attrs.get("finishing", actor.get("current_ability", 50))); var passing := float(attrs.get("passing", actor.get("current_ability", 50))); var dribbling := float(attrs.get("dribbling", attrs.get("pace", actor.get("current_ability", 50)))); var decisions := float(attrs.get("decisions", actor.get("current_ability", 50)))
	var shoot_utility := 95.0 - distance_to_goal * 1.15 - pressure * 30.0 + finishing * 0.32
	...
	shoot_utility += (SeededRngClass.unit_for(seed, 5000 + index) - 0.5) * 10.0
	if shoot_utility >= pass_utility and shoot_utility >= dribble_utility: return "shot"
...
func _resolve_action(actor: Dictionary, team: Array, opponents: Array, action: String, state: Dictionary, seed: int, index: int) -> Dictionary:
	...
	if action == "shot":
		...
		var finishing := float(attrs.get("finishing", actor.get("current_ability", 50))); var composure := float(attrs.get("composure", attrs.get("decisions", actor.get("current_ability", 50)))); var xg := clampf((0.62 - distance / 120.0) * angle_factor - pressure * 0.16, 0.015, 0.62); var execution := clampf(0.75 + (finishing + composure - 100.0) / 350.0, 0.55, 1.25) * _fitness_factor(actor); var scored := SeededRngClass.unit_for(seed, 6000 + index) < clampf(xg * execution, 0.01, 0.85); var on_target := scored or SeededRngClass.unit_for(seed, 6100 + index) < clampf(0.35 + finishing / 250.0 - pressure * 0.12, 0.2, 0.8)
	...
	if action == "pass":
		...
		var positions: Dictionary = state.home_positions if side == "home" else state.away_positions; var target_position: Dictionary = positions[String(receiver.id)]; var distance := SpatialStateClass.distance(state.ball, target_position); var passing := float(attrs.get("passing", actor.get("current_ability", 50))); var technique := float(attrs.get("technique", actor.get("current_ability", 50))); var receiver_pressure := _pressure(state, side, target_position); var success_chance := clampf(0.58 + passing / 300.0 + technique / 500.0 - distance / 180.0 - pressure * 0.18 - receiver_pressure * 0.10, 0.18, 0.96); var success := SeededRngClass.unit_for(seed, 7000 + index) < success_chance * _fitness_factor(actor)
	...
	var direction := 1.0 if side == "home" else -1.0; var target := SpatialStateClass.clamp_position({"x":float(state.ball.x)+direction*6.0,"y":float(state.ball.y)+(SeededRngClass.unit_for(seed, 8100+index)-0.5)*5.0}); var dribbling := float(attrs.get("dribbling", attrs.get("pace", actor.get("current_ability", 50)))); var chance := clampf(0.48 + dribbling / 260.0 - pressure * 0.42, 0.08, 0.9); var retained := SeededRngClass.unit_for(seed, 8000 + index) < chance * _fitness_factor(actor)
...
func _fitness_factor(player: Dictionary) -> float: return 0.65 + 0.35 * clampf(float(player.get("fitness", 100)) / 100.0, 0.0, 1.0)
func _best_receiver(actor: Dictionary, team: Array, state: Dictionary, side: String, seed: int, index: int) -> Dictionary:
	...
	for candidate in team:
		...
		var pos: Dictionary = positions.get(String(candidate.id), actor_pos); var forward := (float(pos.x)-float(actor_pos.x))*direction; var distance := SpatialStateClass.distance(actor_pos, pos); var pressure := _pressure(state, side, pos); var score := forward*0.8 - distance*0.15 - pressure*12.0 + float(candidate.get("current_ability",50))*0.08; score += (SeededRngClass.unit_for(seed, 7200 + index*31 + _stable_key(String(candidate.id)) % 29)-0.5)*2.0
...
func _apply_event(state: Dictionary, event: Dictionary, team: Array, opponents: Array, seed: int, index: int) -> void:
	...
	if String(event.type) == "pass" and bool(event.success): state.ball_owner_id = String(event.receiver_id); state.ball = event.to.duplicate(true); _shift_shape(state, side, 1.5); return
