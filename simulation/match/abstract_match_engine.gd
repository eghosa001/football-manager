...
func simulate_match(home_club: Dictionary, away_club: Dictionary, players: Array, seed: int, context: Dictionary = {}) -> Dictionary:
	var home_players: Array = _players_for_club(players, String(home_club.id))
	var away_players: Array = _players_for_club(players, String(away_club.id))
	...
	var starting_home: Array = _ids(home_lineup)
	var starting_away: Array = _ids(away_lineup)
	var match_context: Dictionary = _match_context(context, home_club, away_club)
	match_context["seed"] = seed
	var factors: Dictionary = MatchFactorsClass.new().breakdown(home_club, away_club, players, match_context)
	...
	var substitutions: Array = _planned_substitutions(home_players, away_players, home_lineup, away_lineup, seed)
	...
	for sequence_index in range(BASE_POSSESSIONS):
		...
		var dynamic_home: float = _lineup_strength(home_lineup, minute, int(result.home_goals) - int(result.away_goals), match_context)
		var dynamic_away: float = _lineup_strength(away_lineup, minute, int(result.away_goals) - int(result.home_goals), match_context)
		...
		var score_diff: int = int(result.home_goals) - int(result.away_goals) if side == "home" else int(result.away_goals) - int(result.home_goals)
...
func _simulate_possession(result: Dictionary, side: String, lineup: Array, opponent: Array, minute: int, seed: int, base_key: int, score_diff: int, context: Dictionary) -> void:
	...
	for i in range(pass_attempts):
		stats.passes += 1
		var came_on: bool = int(passer.get("_came_on_minute", 0)) > 0
		var situational: float = _abilities.situational_bonus(passer, {
			"minute": minute,
			"score_diff": score_diff,
			"came_on": came_on,
			"importance": float(context.get("importance", 0.5)),
			"derby": bool(context.get("derby", false)),
			"under_pressure": pass_attempts >= 3,
			"phase": "midfield"
		})
		var pass_probability: float = clampf(0.72 + (_player_quality(passer, minute, score_diff, context) - 50.0) * 0.0025 + situational * 0.0015, 0.56, 0.93)
	...
	var shot_probability: float = clampf(0.38 + (team_quality - opponent_quality) * 0.004, 0.22, 0.55)
	...
	var long_shot: bool = xg <= 0.085
	var one_on_one: bool = xg >= 0.20
	var shot_context: Dictionary = {
		"minute": minute,
		"score_diff": score_diff,
		"came_on": int(shooter.get("_came_on_minute", 0)) > 0,
		"importance": float(context.get("importance", 0.5)),
		"derby": bool(context.get("derby", false)),
		"long_shot": long_shot,
		"one_on_one": one_on_one,
		"phase": "one_on_one" if one_on_one else "shot"
	}
	xg = clampf(xg * _abilities.shot_multiplier(shooter, shot_context), 0.01, 0.62)
	stats.shots += 1
	stats.xg += xg
	var goal_probability: float = clampf(xg * (0.82 + _player_quality(shooter, minute, score_diff, context) / 300.0), 0.01, 0.76)
	var keeper: Dictionary = _goalkeeper(opponent)
	if not keeper.is_empty():
		goal_probability *= 1.0 - _abilities.goalkeeper_goal_reduction(keeper, {
			"minute": minute,
			"importance": float(context.get("importance", 0.5)),
			"derby": bool(context.get("derby", false)),
			"one_on_one": one_on_one,
			"phase": "one_on_one" if one_on_one else "goalkeeping"
		})
	var goal: bool = _unit(seed, base_key + 33) < goal_probability
	var on_target_probability: float = clampf(0.30 + (_player_quality(shooter, minute, score_diff, context) - 50.0) * 0.003, 0.24, 0.58)
	var on_target: bool = goal or _unit(seed, base_key + 34) < on_target_probability
	if on_target: stats.shots_on_target += 1
	if goal:
		stats.goals += 1
	...
	result.events.append({
		...
		"goalkeeper_id": String(keeper.get("id", "")),
		"xg": snappedf(xg, 0.001),
		"long_shot": long_shot,
		"one_on_one": one_on_one,
		"outcome": "goal" if goal else ("saved" if on_target else "off_target"),
		"special_ability": _active_ability_label(shooter, minute, score_diff, context)
	})
...
func _maybe_card(result: Dictionary, possession_side: String, defenders: Array, minute: int, seed: int, base_key: int) -> void:
	...
	var card_chance: float = 0.021 * _abilities.card_probability_multiplier(player)
	if _unit(seed, base_key + 40) >= card_chance: return
	var red_chance: float = 0.035 * (1.35 if _abilities.has(player, SpecialAbilityServiceClass.TEMPERAMENTAL) else 1.0)
	var red: bool = _unit(seed, base_key + 42) < red_chance
	result.stats[defending_side].cards += 1
	if red: result.stats[defending_side].red_cards += 1
	result.events.append({"minute": minute, "type": "card", "side": defending_side, "player_id": player.id, "card": "red" if red else "yellow", "special_ability": "Temperamental" if _abilities.has(player, SpecialAbilityServiceClass.TEMPERAMENTAL) else ""})
...
func _add_sub_plan(result: Array, side: String, squad: Array, lineup: Array, seed: int, key_base: int) -> void:
	...
	var planned_minutes: Array = [58, 65, 72, 79, 84]
	for i in range(count):
		var base_minute: int = int(planned_minutes[i])
		var minute: int = base_minute + _rand_int(seed, key_base + i, -2, 2)
...
func _apply_due_substitutions(result: Dictionary, plan: Array, home_lineup: Array, away_lineup: Array, minute: int) -> void:
	for substitution in plan:
		...
		var lineup: Array = home_lineup if String(substitution.side) == "home" else away_lineup
		for i in range(lineup.size()):
			...
			var event: Dictionary = {"minute":minute,"type":"substitution","side":String(substitution.side),"player_out":String(substitution.player_out),"player_in":String(substitution.player_in)}
...
func _finalize_possession(result: Dictionary) -> void:
	var home_passes: float = float(result.stats.home.passes)
	var away_passes: float = float(result.stats.away.passes)
	var total: float = maxf(home_passes + away_passes, 1.0)
	result.stats.home.possession = snappedf(home_passes / total * 100.0, 0.1)
	result.stats.away.possession = snappedf(100.0 - float(result.stats.home.possession), 0.1)
...
func _finalize_ratings(result: Dictionary, home_players: Array, away_players: Array) -> void:
	...
	for player in home_players:
		if String(player.id) in home_ids: result.ratings[player.id] = _rating_for(String(player.id), "home", result)
	for player in away_players:
		if String(player.id) in away_ids: result.ratings[player.id] = _rating_for(String(player.id), "away", result)

func _rating_for(player_id: String, side: String, result: Dictionary) -> float:
	var rating: float = 6.5
	for event in result.events:
		if String(event.get("player_id", "")) != player_id: continue
		if String(event.get("type", "")) == "shot" and String(event.get("outcome", "")) == "goal": rating += 0.8
		elif String(event.get("type", "")) == "pass" and String(event.get("outcome", "")) == "complete": rating += 0.006
		elif String(event.get("type", "")) == "pass" and String(event.get("outcome", "")) == "incomplete": rating -= 0.012
		elif String(event.get("type", "")) == "card": rating -= 0.25 if String(event.get("card", "")) == "yellow" else 1.0
	var goal_difference: int = int(result.home_goals) - int(result.away_goals)
...
func _player_precedes(a: Dictionary, b: Dictionary) -> bool:
	if int(a.get("current_ability", 0)) != int(b.get("current_ability", 0)): return int(a.get("current_ability", 0)) > int(b.get("current_ability", 0))
...
func _player_quality(player: Dictionary, minute: int = 0, score_diff: int = 0, context: Dictionary = {}) -> float:
	var consistency: float = _abilities.match_consistency_multiplier(player, int(context.get("seed", 0)), 0)
	var base: float = float(player.get("current_ability", 50)) * (0.85 + float(player.get("fitness", 100)) / 1000.0) * (0.96 + float(player.get("morale", 70)) / 1750.0) * consistency
	var came_on: bool = int(player.get("_came_on_minute", 0)) > 0
	var bonus: float = _abilities.situational_bonus(player, {
		"minute": minute,
		"score_diff": score_diff,
		"came_on": came_on,
		"importance": float(context.get("importance",0.5)),
...
func _pick_shooter(lineup: Array, seed: int, key: int) -> Dictionary:
	var attacking: Array = []
	for player in lineup:
		if String(player.get("position", "")) in ["ST", "AML", "AMR", "AMC"]: attacking.append(player)
...
func _match_context(context: Dictionary, home_club: Dictionary, away_club: Dictionary) -> Dictionary:
	var merged: Dictionary = {"is_home": true, "importance": 0.5, "stage": "", "derby": false}
...
func _active_ability_label(player: Dictionary, minute: int, score_diff: int, context: Dictionary) -> String:
	var bonus: float = _abilities.situational_bonus(player, {
		"minute":minute,
		"score_diff":score_diff,
		"came_on":int(player.get("_came_on_minute",0))>0,
		"importance":float(context.get("importance",0.5)),
		"derby":bool(context.get("derby", false)),
		"phase":"shot"
	})
	if bonus <= 0.0: return ""
	var labels: Array = _abilities.labels_for(player)
