...
func build(fixtures: Array) -> Dictionary:
	...
	var biggest := {}
	var highest_scoring := {}
	var streaks := {}; var win_streaks := {}
	var best_unbeaten := {"club_id":"","matches":0,"from":"","to":""}
	var best_wins := {"club_id":"","matches":0,"from":"","to":""}
	var streak_start := {}; var win_start := {}
	var clean_sheets := {}; var total_goals := 0; var home_wins := 0; var away_wins := 0; var draws := 0; var extra_time_matches := 0; var shootouts := 0
	for fixture in played:
		var home := String(fixture.get("home_club_id","")); var away := String(fixture.get("away_club_id","")); var date := String(fixture.get("date",""))
		var hg := int(fixture.get("final_home_goals",fixture.get("home_goals",0))); var ag := int(fixture.get("final_away_goals",fixture.get("away_goals",0)))
		total_goals += hg + ag
		...
		var margin := abs(hg-ag); var total := hg+ag
	...
	var clean_sheet_record := {"club_id":"","matches":0}
...
func _update_streak(club_id: String, unbeaten: bool, won: bool, date: String, streaks: Dictionary, win_streaks: Dictionary, streak_start: Dictionary, win_start: Dictionary, best_unbeaten: Dictionary, best_wins: Dictionary) -> void:
	...
	else: streaks[club_id] = 0; streak_start.erase(club_id)
	if won:
	...
	else: win_streaks[club_id] = 0; win_start.erase(club_id)
