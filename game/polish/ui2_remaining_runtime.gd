...
func _header(parent: VBoxContainer, title: String, subtitle: String, tabs: TabContainer, app: Node) -> void:
	var bar := HBoxContainer.new(); bar.custom_minimum_size.y = 62; bar.add_theme_constant_override("separation", 10); parent.add_child(bar)
	var heading := UI.title(bar, title, subtitle); heading.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var continue_button := UI.action("CONTINUE  ›", UI.CYAN); continue_button.custom_minimum_size.x = 155; continue_button.pressed.connect(func() -> void: app.call("_advance_day")); bar.add_child(continue_button)
	UI.divider(parent)
