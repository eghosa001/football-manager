...
func _hook_continue(button: Button) -> void:
	if button.has_meta("live_match_continue") or not button.visible: return
	var text := button.text.strip_edges().to_upper()
	if not text.begins_with("CONTINUE"): return
...
func _open_match_window(app: Node, coordinator) -> void:
	...
	window.close_requested.connect(func(): window.queue_free(); _active_window = null)
	app.add_child(window)
	var margin := MarginContainer.new(); margin.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); margin.add_theme_constant_override("margin_left",12); margin.add_theme_constant_override("margin_top",10); margin.add_theme_constant_override("margin_right",12); margin.add_theme_constant_override("margin_bottom",10); window.add_child(margin)
	var root := VBoxContainer.new(); root.add_theme_constant_override("separation",8); margin.add_child(root)
	var scoreboard := Label.new(); scoreboard.add_theme_font_size_override("font_size",24); scoreboard.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; root.add_child(scoreboard)
	var body := HBoxContainer.new(); body.add_theme_constant_override("separation",12); body.size_flags_vertical = Control.SIZE_EXPAND_FILL; root.add_child(body)
	var viewer := MatchViewer.new(); viewer.custom_minimum_size = Vector2(760,520); viewer.size_flags_horizontal = Control.SIZE_EXPAND_FILL; viewer.size_flags_vertical = Control.SIZE_EXPAND_FILL; body.add_child(viewer)
	var controls := VBoxContainer.new(); controls.custom_minimum_size.x = 330; controls.add_theme_constant_override("separation",7); body.add_child(controls)
	var status := Label.new(); status.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; controls.add_child(status)
	var time_row := HBoxContainer.new(); controls.add_child(time_row)
	...
	var formation := OptionButton.new(); var formations: Array=Tactics.FORMATIONS.keys(); formations.sort(); for value in formations: formation.add_item(String(value)); controls.add_child(formation)
	var mentality := OptionButton.new(); for value in Tactics.VALID_MENTALITIES: mentality.add_item(String(value).capitalize()); mentality.set_item_metadata(mentality.item_count-1,String(value)); controls.add_child(mentality)
	var apply_tactic := Button.new(); apply_tactic.text="Apply tactic now"; controls.add_child(apply_tactic)
	var events_label := Label.new(); events_label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; events_label.size_flags_vertical=Control.SIZE_EXPAND_FILL; controls.add_child(events_label)
	...
	var refresh := func(message: String = ""):
		...
		for id in lineup:
			off.add_item(_player_label(app.get("session").world,String(id))); off.set_item_metadata(off.item_count-1,String(id))
		for id in bench:
			on.add_item(_player_label(app.get("session").world,String(id))); on.set_item_metadata(on.item_count-1,String(id))
		make_sub.disabled = bool(snap.get("finished",false)) or off.item_count==0 or on.item_count==0
		plus5.disabled=bool(snap.get("finished",false)); plus10.disabled=bool(snap.get("finished",false)); interval.disabled=bool(snap.get("finished",false)); full_time.disabled=bool(snap.get("finished",false)); apply_tactic.disabled=bool(snap.get("finished",false)); commit.disabled=not bool(snap.get("finished",false))
		...
		events_label.text="\n".join(lines)
	refresh.call()
	plus5.pressed.connect(func(): coordinator.advance(int(coordinator.snapshot().get("minute",0))+5); refresh.call())
	plus10.pressed.connect(func(): coordinator.advance(int(coordinator.snapshot().get("minute",0))+10); refresh.call())
	interval.pressed.connect(func():
		var minute:=int(coordinator.snapshot().get("minute",0)); var target:=45 if minute<45 else mini(90,minute+15); coordinator.advance(target); refresh.call()
	)
	...
	apply_tactic.pressed.connect(func():
		var snap:Dictionary=coordinator.snapshot(); var side:=String(snap.get("managed_side","home")); var current:Dictionary=snap.get("tactics",{}).get(side,{})
		...
		var err:=coordinator.change_tactic(tactic); refresh.call("Tactical change applied." if err==OK else "Tactical change failed (%d)."%err)
	)
	full_time.pressed.connect(func(): coordinator.advance(90); refresh.call())
...
func _autosave_if_due(app: Node) -> void:
	var session=app.get("session"); var settings:Dictionary=app.get("settings"); var slot:=int(app.get("active_slot"))
...
func _event_text(event: Dictionary, world: Dictionary) -> String:
	var minute:=int(event.get("minute",0)); var type:=String(event.get("type","event"))
	if type=="substitution":return "%d' SUB: %s → %s"%[minute,_player_label(world,String(event.get("player_out",""))),_player_label(world,String(event.get("player_in","")))]
	if type=="tactical_change":return "%d' Tactics: %s %s"%[minute,String(event.get("formation","")),String(event.get("mentality",""))]
	if type=="shot" and String(event.get("outcome",""))=="goal":return "%d' GOAL — %s"%[minute,_player_label(world,String(event.get("player_id","")))]
	if type=="card":return "%d' %s card — %s"%[minute,String(event.get("card","" )).capitalize(),_player_label(world,String(event.get("player_id","")))]
	return "%d' %s"%[minute,type.replace("_"," ").capitalize()]

func _player_names(world: Dictionary) -> Dictionary:
	var result:={}
	for player in world.get("players",[]):result[String(player.get("id",""))]=_player_label(world,String(player.get("id","")))
	return result

func _player_label(world: Dictionary,id: String) -> String:
	for player in world.get("players",[]):
		if String(player.get("id",""))!=id:continue
		var name:=String(player.get("name","")).strip_edges(); if name=="":name=(String(player.get("first_name",""))+" "+String(player.get("last_name",""))).strip_edges()
