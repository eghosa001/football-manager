...
func _complete_history_tab(tabs: TabContainer) -> void:
	...
	_add_rows(box, history.get("biographies", []), func(row): return "%s — %s" % [String(row.get("name","")), String(row.get("summary",""))])
...
func _add_rows(parent: VBoxContainer, rows: Array, formatter: Callable) -> void:
	if rows.is_empty():
		var empty := Label.new(); empty.text = tr("No historical records yet."); parent.add_child(empty); return
	for row in rows:
		var label := Label.new(); label.text = String(formatter.call(row)); label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; parent.add_child(label)
...
func _on_match_frame(_index: int, minute: int, viewer: Node) -> void:
	...
	for event in result.get("events", []):
		if String(event.get("type","")) != "substitution" or int(event.get("minute",-1)) != minute: continue
		var key := "%s:sub:%d:%s:%s" % [str(viewer.get_instance_id()),minute,String(event.get("player_out","")),String(event.get("player_in",""))]
		if _seen_events.has(key): continue
		_seen_events[key] = true; _substitution_cue()

func _substitution_cue() -> void:
...
func _play_tone(frequency: float, seconds: float, amplitude: float) -> void:
	...
	if polish != null:
		...
		if typeof(settings)==TYPE_DICTIONARY and bool(settings.get("mute",false)): return
...
func _tone(frequency: float, seconds: float, amplitude: float) -> AudioStreamWAV:
	...
	for i in range(samples):
		var envelope := 1.0-float(i)/float(samples); var value := int(sin(TAU*frequency*float(i)/float(rate))*amplitude*envelope*32767.0); bytes.encode_s16(i*2,value)
...
func _ordinal_suffix(value: int) -> String:
	...
	if mod100 in [11,12,13]: return "th"
	match value%10:
		1: return "st"
		2: return "nd"
		3: return "rd"
