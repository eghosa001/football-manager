...
func _apply_accessibility(control: Control) -> void:
	# Keep all actionable controls keyboard reachable and expose their visible label
	# as a tooltip for assistive technologies that consume control descriptions.
...
func _on_match_frame(_index: int, minute: int, viewer: Node) -> void:
	...
	for event in result.get("events", []):
		if String(event.get("type", "")) != "substitution": continue
...
func _whistle(repeats: int = 1) -> void:
	# A higher, short tone remains distinguishable from the goal/UI cues even
	# when the game runs without bundled binary audio assets.
...
func _play_tone(frequency: float, seconds: float, amplitude: float) -> void:
	...
	var stream := _tone(frequency, seconds, amplitude)
