...
func audit(world: Dictionary) -> Dictionary:
	...
	stats["clubs"] = world.get("clubs",[]).size(); stats["players"] = world.get("players",[]).size(); stats["competitions"] = world.get("competitions",[]).size(); stats["fixtures"] = world.get("fixtures",[]).size(); stats["contracts"] = world.get("contracts",[]).size()
