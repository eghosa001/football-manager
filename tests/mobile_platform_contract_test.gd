...
func _initialize() -> void:
	...
	_check(bool(ProjectSettings.get_setting("input_devices/pointing/emulate_mouse_from_touch", false)), "touch input can drive mouse-oriented controls")
	...
	if file != null:
		...
		_check(presets.contains("name=\"Android\""), "Android test export preset exists")
		_check(presets.contains("architectures/arm64-v8a=true"), "Android ARM64 export is enabled")
		_check(presets.contains("name=\"Android Play\""), "Google Play export preset exists")
		_check(presets.contains("gradle_build/use_gradle_build=true"), "Google Play export uses Gradle")
		_check(presets.contains("gradle_build/export_format=1"), "Google Play export uses AAB format")
