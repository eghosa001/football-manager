...
func _init() -> void:
	...
	var world: Dictionary = GeneratorClass.new().create_world(6601,4,20,25)
	var history:Array=[]
	var cycle=CareerCycleClass.new()
	var initial_players: int = int(world.players.size()); var initial_clubs: int = int(world.clubs.size())
	var min_players: int = initial_players; var max_players: int = initial_players; var negative_cash_seasons: int = 0
	...
	var player_ratio: float = float(world.players.size())/maxf(1.0,float(initial_players))
	var ok: bool = world.clubs.size()==initial_clubs and player_ratio>=0.55 and player_ratio<=2.4 and min_players>=int(initial_players*0.45) and max_players<=int(initial_players*2.8) and negative_cash_seasons<=int(seasons*0.35)
