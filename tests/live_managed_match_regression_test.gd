...
func _init() -> void:
	...
	var started: Dictionary = match.start_match(home,away,players,919191,{})
	assert(int(started.get("error",FAILED)) == OK)
	match.advance_to_minute(45)
	var half: Dictionary = match.snapshot()
