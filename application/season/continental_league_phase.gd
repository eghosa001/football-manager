...
func initialize(world: Dictionary, competition: Dictionary, season_year: int) -> void:
	...
	clubs.sort()
	var matches_per_club: int = int(competition.get("modern_league_phase",{}).get("matches_per_club",8))
...
func advance_ready(world: Dictionary, competition_id: String, current_date: String) -> Dictionary:
	var competition: Dictionary = _competition(world,competition_id)
	if competition.is_empty() or String(competition.get("competition_type","")) != "continental_league_phase": return {}
	var stage: String = String(competition.get("stage","league_phase"))
	if stage == "complete": return competition
	var fixtures: Array = _stage_fixtures(world,competition_id,stage)
...
func record(world: Dictionary, competition: Dictionary) -> Dictionary:
	...
	return {"competition_id":String(competition.get("id","")),"competition_name":String(competition.get("name","")),"season_start_year":int(competition.get("season_year",world.get("season_year",2026))),"tier":int(competition.get("continental_tier",1)),"complete":String(competition.get("stage","")) == "complete","fixture_count":fixtures.size(),"table":competition.get("league_phase_table",[]).duplicate(true),"champion_club_id":String(competition.get("champion_club_id","")),"competition_type":"continental_league_phase","stage_history":competition.get("stage_history",[]).duplicate(true)}
...
func _append_league_phase(world: Dictionary, competition: Dictionary, clubs: Array, matches_per_club: int, season_year: int) -> void:
	...
	var edges: Array = _selected_round_robin_edges(clubs,matches_per_club)
	_orient_even_degree_edges(edges,clubs)
	var base: String = "%04d-09-17" % season_year
	for edge_index in range(edges.size()):
		var edge: Dictionary = edges[edge_index]
		var round_no: int = int(edge.round)
		var date: String = _add_days(base,(round_no-1)*21)
		world.fixtures.append({"id":"elp-%s-%d-r%d-%d" % [String(competition.id),season_year,round_no,edge_index],"competition_id":String(competition.id),"round":round_no,"stage":"league_phase","league_phase":true,"home_club_id":String(edge.home),"away_club_id":String(edge.away),"played":false,"home_goals":0,"away_goals":0,"date":date,"season_year":season_year,"away_goals_rule":false})

func _selected_round_robin_edges(clubs: Array, round_count: int) -> Array:
	var teams: Array = clubs.duplicate()
	if teams.size() % 2 != 0: teams.append("")
	var count: int = teams.size()
	var edges: Array = []
	for round_index in range(round_count):
		for pair_index in range(int(count / 2)):
			var a: String = String(teams[pair_index]); var b: String = String(teams[count-1-pair_index])
		...
		teams = [fixed_team]; teams.append_array(rotating)
	return edges
...
func _orient_even_degree_edges(edges: Array, clubs: Array) -> void:
	...
	for edge_index in range(edges.size()):
		...
		var current: String = String(edges[edge_index].a)
		while true:
			var next_edge: int = -1
			...
			var other: String = String(edge.b) if String(edge.a) == current else String(edge.a)
...
func _finish_league_phase(world: Dictionary, competition: Dictionary, current_date: String) -> void:
	var fixtures: Array = _stage_fixtures(world,String(competition.id),"league_phase")
	...
	for i in range(table.size()):
		var club_id: String = String(table[i].club_id)
	...
	if playoff.size() < 2:
		...
		return
	var pairings: Array = []
	for i in range(int(playoff.size()/2)): pairings.append({"seeded":String(playoff[i]),"unseeded":String(playoff[playoff.size()-1-i])})
...
func _finish_two_leg_stage(world: Dictionary, competition: Dictionary, stage: String, current_date: String) -> void:
	var fixtures: Array = _stage_fixtures(world,String(competition.id),stage)
	var ties: Dictionary = {}
	for fixture in fixtures:
		var tie_id: String = String(fixture.get("tie_id",""))
	...
	for tie_id in ties.keys():
		...
		var club_a: String = String(first.get("away_club_id","")); var club_b: String = String(first.get("home_club_id",""))
		var a_goals: int = int(first.get("away_goals",0)) + int(second.get("home_goals",0)); var b_goals: int = int(first.get("home_goals",0)) + int(second.get("away_goals",0))
		var winner: String = ""
		if a_goals != b_goals:
			winner = club_a if a_goals > b_goals else club_b
		else:
			var resolution: Dictionary = ResolutionClass.new().resolve_level_aggregate(world,second,competition,_stable_seed(String(second.id),int(competition.get("season_year",0))))
			for key in resolution.keys(): second[key] = resolution[key]
			winner = String(resolution.get("winner",""))
		second["aggregate_home_goals"] = a_goals; second["aggregate_away_goals"] = b_goals; second["aggregate_winner"] = winner
		winners.append(winner)
	competition.stage_history.append({"stage":stage,"winners":winners.duplicate()})
	var next_stage: String = _next_stage(stage)
	competition["stage"] = next_stage
	if next_stage == "round_of_16":
		var direct: Array = competition.get("direct_round_of_16",[]).duplicate(); var pairings: Array = []
		...
		_append_two_leg_stage(world,competition,next_stage,pairings,_add_days(current_date,21))
	elif next_stage == "final": _append_final(world,competition,winners,_add_days(current_date,21))
...
func _finish_final(world: Dictionary, competition: Dictionary, fixtures: Array) -> void:
	...
	var home: String = String(fixture.get("home_club_id","")); var away: String = String(fixture.get("away_club_id","")); var winner: String = ""
	if int(fixture.home_goals) != int(fixture.away_goals): winner = home if int(fixture.home_goals)>int(fixture.away_goals) else away
	else:
		var resolution: Dictionary = ResolutionClass.new().resolve_level_match(world,fixture,competition,_stable_seed(String(fixture.id),int(competition.get("season_year",0))))
		for key in resolution.keys(): fixture[key] = resolution[key]
		winner = String(resolution.get("winner",""))
	competition["champion_club_id"] = winner; competition["stage"] = "complete"
...
func _append_two_leg_stage(world: Dictionary, competition: Dictionary, stage: String, pairings: Array, first_date: String) -> void:
	var season_year: int = int(competition.get("season_year",world.get("season_year",2026)))
	for i in range(pairings.size()):
		var seeded: String = String(pairings[i].seeded); var unseeded: String = String(pairings[i].unseeded)
		var tie_id: String = "%s-%s-%d" % [String(competition.id),stage,i]
		var date1: String = _conflict_free_date(world,unseeded,seeded,first_date); var date2: String = _conflict_free_date(world,seeded,unseeded,_add_days(date1,7))
...
func _append_final(world: Dictionary, competition: Dictionary, winners: Array, date_string: String) -> void:
	...
	var season_year: int = int(competition.get("season_year",world.get("season_year",2026))); var home: String = String(winners[0]); var away: String = String(winners[1]); var date: String = _conflict_free_date(world,home,away,date_string)
	var row: Dictionary = _fixture("%s-final-%d" % [String(competition.id),season_year],String(competition.id),"final",1,home,away,date,season_year,"final",1)
	row["neutral_venue"] = true; row["single_leg"] = true; world.fixtures.append(row)
...
func _fixture(id: String, competition_id: String, stage: String, round_no: int, home: String, away: String, date: String, season_year: int, tie_id: String, leg: int) -> Dictionary:
	return {"id":id,"competition_id":competition_id,"stage":stage,"round":round_no,"home_club_id":home,"away_club_id":away,"date":date,"season_year":season_year,"played":false,"home_goals":0,"away_goals":0,"continental_knockout":true,"tie_id":tie_id,"leg":leg,"away_goals_rule":false,"extra_time":leg==2,"penalties":leg==2}
...
func _add_days(date_string: String, days: int) -> String:
	var p: PackedStringArray = date_string.split("-")
