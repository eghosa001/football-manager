...
func advance_ready(world: Dictionary, competition_id: String, current_date: String) -> Dictionary:
	...
	var round_number: int = int(bracket.get("round", 1))
	var fixtures: Array = _round_fixtures(world, competition_id, round_number)
	...
	for i in range(bracket.get("matches", []).size()):
		...
		var row: Dictionary = {"home_goals":int(fixture.get("home_goals",0)),"away_goals":int(fixture.get("away_goals",0))}
	...
	if bool(next.get("complete", false)):
		...
		return next
	var next_date: String = _add_days(current_date, 14)
...
func _resolve_tied_fixture(world: Dictionary, competition: Dictionary, fixture: Dictionary, pairing: Dictionary) -> void:
	if not bool(competition.get("extra_time", true)):
		...
		return
	var resolution: Dictionary = ResolutionClass.new().resolve_level_match(world, fixture, competition, _stable_seed(String(fixture.get("id","")), int(competition.get("season_year",0))))
...
func _append_round_fixtures(world: Dictionary, competition: Dictionary, bracket: Dictionary, season_year: int, date_string: String) -> void:
	for fixture in world.fixtures:
		if String(fixture.get("date", "")) == "": fixture["date"] = _add_days("%04d-08-01" % season_year, (int(fixture.get("round", 1)) - 1) * 7)
	for i in range(bracket.get("matches", []).size()):
		var pairing: Dictionary = bracket.matches[i]
		var home: String = String(pairing.get("home", "")); var away: String = String(pairing.get("away", ""))
		if home == "" or away == "": continue
		var candidate: String = date_string
		while _has_conflict(world, home, away, candidate): candidate = _add_days(candidate, 1)
		date_string = candidate
		world.fixtures.append({"id":"cup-%s-%d-r%d-m%d" % [String(competition.get("id", "cup")),season_year,int(bracket.get("round",1)),i],"competition_id":String(competition.get("id","")),"round":int(bracket.get("round",1)),"bracket_index":i,"home_club_id":home,"away_club_id":away,"played":false,"home_goals":0,"away_goals":0,"date":date_string,"season_year":season_year,"knockout":true,"stage":"knockout","extra_time":true,"penalties":true,"away_goals_rule":false})
...
func _add_days(date_string: String, days: int) -> String:
	var pieces: PackedStringArray = date_string.split("-")
