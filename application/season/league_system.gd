...
func apply_promotion_relegation(world: Dictionary, season_records: Array, places: int = 3) -> Array:
	...
	for country_id in by_country.keys():
		...
		for i in range(leagues.size() - 1):
			var upper: Dictionary = leagues[i]; var lower: Dictionary = leagues[i+1]
			if not records_by_competition.has(String(upper.id)) or not records_by_competition.has(String(lower.id)): continue
			var upper_record: Dictionary = records_by_competition[String(upper.id)]; var lower_record: Dictionary = records_by_competition[String(lower.id)]
			if not bool(upper_record.get("complete",false)) or not bool(lower_record.get("complete",false)): continue
			var configured := mini(int(upper.get("relegation_places", places)), int(lower.get("promotion_places", places)))
			var move_count: int = mini(configured, mini(upper.club_ids.size(), lower.club_ids.size()))
			if move_count <= 0: continue
			var relegated: Array = []; var promoted: Array = []
			...
			for club_id in relegated: upper.club_ids.erase(club_id); lower.club_ids.append(club_id)
			for club_id in promoted: lower.club_ids.erase(club_id); upper.club_ids.append(club_id)
			movements.append({"country_id":country_id,"upper_competition_id":upper.id,"lower_competition_id":lower.id,"promoted":promoted,"relegated":relegated})
	return movements
