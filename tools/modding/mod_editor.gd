...
func set_metadata(mod: Dictionary, key: String, value) -> Dictionary:
	...
	if key not in ["name","version","author","dependencies","conflicts"]: return {"ok":false,"error":"unsupported_metadata_field","mod":mod.duplicate(true)}
...
func add_patch(mod: Dictionary, collection: String, entity_id: String, values: Dictionary) -> Dictionary:
	...
	candidate["patches"] = candidate.get("patches", {})
...
func export_mod(path: String, mod: Dictionary) -> Error:
	...
	file.store_string(JSON.stringify(exportable, "\t", false)); file.flush(); file.close()
	return OK
